...
import { updateCameraAction, UpdateCameraData } from "../actions/updateCamera.action";
...
export function EditCameraModal({ open, onClose, onSuccess, camera }: EditCameraModalProps) {
  ...
  // Load alert types and populate form when modal opens or camera changes
  useEffect(() => {
    if (open && camera) {
      setFormData({
        name: camera.name,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await updateCameraAction(camera.id, updateData);

      if (result.success) {
        onSuccess();
        handleClose();
      } else {
        setError(result.error || "Failed to update camera");
      }
    } catch (err: unknown) {
    ...
    >
      <DialogTitle>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
          Update camera configuration and alert settings
        </Typography>
      </DialogTitle>
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={12}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                Enable or disable alerts that this camera should monitor.
              </Typography>
